...
允许的页面类型：`source`、`entity`、`concept`、`query`、`synthesis`、`comparison`、`maintenance`。

每个 wiki 页面都必须在文件顶部包含且只包含一个 YAML frontmatter。

```yaml
---
type: source | entity | concept | query | synthesis | comparison | maintenance
title: 页面标题
sources: []
tags: []
created_at: ""
...
由资料生成的页面必须在 `sources` 字段记录来源。source summary 页面记录 raw source 路径；概念、实体、对比、综合和问答页面记录支持该页面的 source id，并在正文中尽量链接到相关 source summary。
...
Allowed page types: `source`, `entity`, `concept`, `query`, `synthesis`, `comparison`, `maintenance`.
