...
def test_structural_lint_detects_orphan_no_outlinks_and_broken_links(tmp_path: Path) -> None:
    ...
    _write(paths.wiki_concepts_dir / "Transformer.md", "Links to [[Attention Mechanism|attention]] and [[Missing Page]].")
    _write(paths.wiki_concepts_dir / "Attention Mechanism.md", "No links here.")
    _write(paths.wiki_entities_dir / "OpenAI.md", "No links here either.")
    ...
    assert any(issue.page == "concepts/Transformer.md" and "[[Missing Page]]" in issue.detail for issue in by_type["broken-link"])
    assert {issue.page for issue in by_type["orphan"]} == {"overview.md", "concepts/Transformer.md", "entities/OpenAI.md"}
    assert {issue.page for issue in by_type["no-outlinks"]} == {"overview.md", "concepts/Attention Mechanism.md", "entities/OpenAI.md"}
...
PAGES: concepts/rag.md, sources/paper-a.md
---END LINT---
...
"""
    ...
    assert issues[0].affectedPages == ["concepts/rag.md", "sources/paper-a.md"]
...
---END LINT---"""
    ...
    assert "### concepts/rag.md" in prompt
...
def test_lint_wiki_includes_semantic_only_when_requested(tmp_path: Path) -> None:
    paths = create_wiki_database(str(tmp_path / "wiki"), title="Research Wiki")
    _write(paths.wiki_concepts_dir / "lonely.md", "No links.")
